...
def main(argv=None) -> None:
    ...
    res = triangulate_epsilon(cfg, seed=args.seed, n_episodes=args.episodes)

    print(f"reference spread h_ref = {res.h_ref:.4f} (analytic fixed point h*)")
    print(f"  analytic  epsilon        = {res.epsilon_analytic:8.4f}   (theory 1.1)")
    print(f"  BR-slope  epsilon_hat    = {res.epsilon_br:8.4f}   (m_hat={res.modulus_br:.3f} x gamma/beta)")
