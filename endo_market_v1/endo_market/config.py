"""Configuration schema and loading for the endo_market project.
...
class ClientsConfig:
    ...
    info_base_intensity: float = 0.40  # alpha-independent baseline toxic level (sets response regime)
    info_cap: float = 2.5  # saturation cap on informed notional (bounds the toxicity)
...
class SimulatorConfig:
    ...
    horizon: int = 64
    fundamental_vol: float = 0.10  # per-step random-walk vol of latent fundamental
    ...
    init_mispricing_vol: float = 0.3  # std of initial mid-vs-fundamental gap
