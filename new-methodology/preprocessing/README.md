...
Large generated files are excluded from git (see `.gitignore`) to keep the repo under GitHub's size limit. Run the pipeline to regenerate them:

```bash
python src/fetch_data.py       # 1. download raw sources from public GitHub mirrors
python src/build_datasets.py   # 2. build processed/ and master/ files
python src/verify_data.py      # 3. verify all 33 checks pass
python src/preprocess.py       # 4. run full preprocessing pipeline
```

This takes ~2 minutes and regenerates everything. Files committed to the repo (small reference data, scripts, docs) are immediately usable without running the pipeline.
