...
- [x] Confirm paper fits within **8 pages total** (two-column ACM `sigconf` format), including all figures and references - **v2** compiled on Overleaf 2026-08-07 at **exactly 8 pages** ([`paper/REFLEX_Research_Paper.pdf`](paper/REFLEX_Research_Paper.pdf)), all six figures and five tables placed and the full 24-entry bibliography inside the limit; v1's compile is kept at [`paper/archive/`](paper/archive/). Slack is still zero, so re-check after any edit (trim order in `paper/README.md`)
